    ...
    public class Exercises01
    {
        ...
        public async Task GetDataForUser3_CheckServerHeader_ShouldBeCloudflare()
        {
             ...
             */
             ...
             string altServerHeaderValue = response.GetHeaderValue("Server") ?? string.Empty;

             Assert.That(serverHeaderValue, Is.EqualTo("cloudflare"));
             Assert.That(altServerHeaderValue, Is.EqualTo("cloudflare"));
        }
        ...
        public async Task GetDataForUser4_CheckName_ShouldBePatriciaLebsack()
        {
             ...
             */
             ...
             JObject bodyData = JObject.Parse(response.Content!);

             Assert.That(bodyData.SelectToken("name")!.ToString(), Is.EqualTo("Patricia Lebsack"));
        ...
        public async Task GetDataForUser5_CheckCompanyName_ShouldBeKeeblerLLC()
        {
             ...
             */
             ...
             JToken company = bodyData.SelectToken("company")!;

             Assert.That(bodyData.SelectToken("company.name")!.ToString(), Is.EqualTo("Keebler LLC"));
        }
    }
}
